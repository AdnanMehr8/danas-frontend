...
const MixingCream = () => {
  ...
  useEffect(() => {
    const storedRecord = JSON.parse(localStorage.getItem("sc-mixingRecord"));
    if (storedRecord) {
      dispatch(setscMixingRecord(storedRecord));
    }
  }, [dispatch]);
    ...
    switch (tabValue) {
      ...
      case 2: // Combined case for coating solution preparation and coating procedure
        if (
          ...
          !manufacturingRecord[0].meltingStartedAt ||
          !manufacturingRecord[0].meltingCompletedOn ||
  ...
  const handleNextTab = () => {
    if (validateFields()) {
      dispatch(setscMixingRecord(record));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data && data._id) {
        localStorage.setItem("sc-mixingId", data._id);
        console.log("MixingID stored in localStorage:", data._id);
      }

      const processes = JSON.parse(localStorage.getItem("processes"));
      if (processes) {
        const currentProcessIndex = processes.indexOf("mixing-cream");
        if (
